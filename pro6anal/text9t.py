...
# 두 집단의 평균과 표준편차 비율에 대한 제조 검정법이다.

# 두 집단의 가설검정 실습 시 분산을 알지 못하는 것으로 한정하겠다.
# 서로 독립인 두 집단의 평균 차이 검정 (independant two-sample t-test)
...
male = [75, 85, 100, 72.5, 86.5]    # 83.8  
female = [63.2, 76, 52, 100, 70]    # 72.24
...
# 해석 : pvalue 0.2525 > 0.05  귀무채택.    통계적으로 유의하지 않다.

# 선행 조건
# 1) 두 집단이 각각 정규분포를 따라야한다.
# 2) 두 집단의 분산이 같다는 가정이 필요. 등분 산성
print('male 의 정규성 : ', stats.shapiro(male))     # 0.60037140 > 0.05 정규성 만족
print('female 의 정규성 : ', stats.shapiro(female)) # 0.77804311 > 0.05
# 만약 집단의 표본 수가 30개 이상인 경우는 정규분포를 따른다고 가정함으로 정규성 검정 안해도 됨
# 만약 정규성을 만족하지 못하면 비모수 검정인 Mann-whitney test를 한다.
# stats.mannwhitneyu(group1, group2)     p > 0.05 인 경우 차이 증거 없음 아니면 두 집단 평균 차이 없음

print()
# 선행 조건 2) 두 집단의 분산이 같다는 가정이 필요. 등분산성(데이터의 퍼짐 정도)
from scipy.stats import levene, bartlett
# levene    정규성과 상관없이 사용, 이상치에 민감, 어떤 분포도 사용 가능                   
# bartlett  정규성이 있을 때만 사용, 이상치에 덜 민감, 정규분포일 때만 사용 가능
leven_stat, leven_p = levene(male, female)
print('p value : ', leven_p)    # 0.49565116 >= 0.05 등분산성 만족
# two_sample = stats.ttest_ind(male, female, equal_var=True)    # 이거 사용?
# 만약 등분산성 만족 못할 때 Welch's t-test수행
two_sample = stats.ttest_ind(male, female, equal_var=False)
print(two_sample)
# TtestResult : statistic=1.23319, pvalue0.259533, df=6.61303386
