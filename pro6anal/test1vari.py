...
plt.scatter(x1, class1, alpha=0.8, 
            label=f"1반(평균={mean1:2f},표준편차={std1:2f})")

plt.scatter(x2, class2, alpha=0.8, 
            label=f"1반(평균={mean2:2f},표준편차={std2:2f})")

plt.hlines(target_mean, 0.5, 0.25, colors='red',
...
plt.axvline(target_mean, color='red', linestyle='dotted', 
            label=f"공통평균={target_mean}")
plt.xlabel('시험 점수')
plt.ylabel("빈도")
plt.legend()
plt.tight_layout()
...
# 국어 선생님 입장
# 귀무 가설(전통적 주장) : 두 반의 국어 점수의 표준편차(OR 평균)는 차이가 없다.
# 누군가가 실험을 통해 데이터 수집 후 두 반의 점수에 통계 계산 후 새로운 의견(주장)
# 대립 가설 : 두 반의 국어 점수의 표준편차(평균)는 차이가 있다.
# 가설검정(t-test)을 통해 두 의견의 채택, 기각을 판단할 수 있음.
