...
# 태아수 PR(>F) 1.051039e-27 : 귀무기각 태아 수와 태아의 머리둘레 평균은 차이가 있다.
# 관측자수 PR(>F) 6.497055e-03(0.006) : 귀무기각, 관측자 수와 태아의 머리둘레 평균은 관련이 있다.
# 상호작용 pr(>F)  3.295509e-01(0.3) : 귀무채택. 태아수와 관측자수 관련이 없다.
# 해석 :
# 태아수와 관측자수는 각각 종속변수에 유의한 영향을 미친다.
...
주효과 가설 :
    ...
    귀무 : treat(치료) 방법과 독 퍼짐 시간의 평균은 차이가 없다.
    대립 : treat(치료) 방법과 독 퍼짐 시간의 평균은 차이가 있다.
교호 작용 가설
...
# 요인별 레벨의 표본수는 4로 동일 (모든 집단 별 표본수 동일하므로 균형설계가 잘됨)

result2 = ols('time ~ C(poison) * C(treat)', data=data2).fit() # fit() <- 학습시킴
...
# poison PR(>F) 3.331440e-07 < 0.05 : 귀무기각
# treat PR(>F) 3.777331e-06 < 0.05 : 귀무기각
# 상호작용 pr(>F) 1.122506e-01 > 0.05 : 귀무채택
# 해석 :
# poison 종류와 독 퍼짐 시간의 평균은 차이가 있다.
# treat(치료) 방법과 독 퍼짐 시간의 평균은 차이가 있다.
# 교호작용이 없다 (poison 종류와 응급처치 방법은 관련이 없다)
# 주효과는 있지만 상호작용은 없음
...
print(tkResult2)
tkResult1.plot_simultaneous(xlabel='mean of time', ylabel='treat')
plt.show()
plt.close()