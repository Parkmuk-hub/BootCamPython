...
# 독립성 : 동일 집단의 두 변인 학력수준과 대학진학 여부를 대상으로 관련성이 있는가
# 독립성 검정은 두 변수 사이의 연관성을 검정한다

# 실습
# 교육수준(독립변수, x)과 흡현율(종속변수, y) 간의 관련성 분석

# 귀무 : 교육수준과 흡연율 간에 관계가 있다. (독립이다. 연관성이 없다)
...
# chi2:18.9109157, p:0.000818257, dof:4
print("expected : \n", expected)    # 예측된 기대도수

# 판정1 : 유의수준 : 0.05 > p:0.000818257 이므로 가설이 기각
# 교육수준과 흡연율 간에 관계가 있다. smoke.csv(수집자료)는 우연히 발생된 자료가 아니다

# 판정2 : chi:18.l910915, dof:4, critical value:9.49
# chi2 값이 임계치 우측에 있으므로 귀무가설이 기각되고 대립가설이 채택

# 이후 다향한 자료, 의견 등으로 보고서를 작성

print('-----독립성 검정 : 실습2 -------------------')
# 남성과 여성의 스포츠 음료 선호도 검정

# 귀무(H0) : 성별과 음료 선호는 서로 관련이 없다.
# 대립(H1) : 성별과 음료 선호는 서로 관련이 있다.
...
# 유의수준 0.05 > p:0.033880 이므로 귀무가설 기각
# 성별과 음료 선호는 서로 관련 있다.
