...
# 정규성 확인
print(stats.shapiro(gr1).pvalue)    # 0.333682 > 0.05
print(stats.shapiro(gr2).pvalue)    # 0.656105 > 0.05
print(stats.shapiro(gr3).pvalue)    # 0.832481 > 0.05

# 등분산성
print(stats.levene(gr1, gr2, gr3).pvalue)   # 0.045846 < 0.05
...
print(df)
lmodel = ols('pay ~ C(group)', data=df).fit()   # C(group)은 범주형
print(anova_lm(lmodel, typ=1))
# p 0.043589 < alpha 0.05 이므로 귀무 기각

# 일원분산분석 방법 2 : f_oneway()
f_stat, p_val = stats.f_oneway(gr1, gr2, gr3)
print('f_stat : ', f_stat)
print('p_val : ', p_val)    # 0.0435893
print()
...
# 참고
# anova_lm() : 정규형, 등분성이 깨지면 p-value 신뢰불가
# f_oneway() : 정규성 깨지면 welch ANOVA, 등분산성이 깨지면 stats.kruskal() 사용