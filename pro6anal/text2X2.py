...
# 범주형 자료(변수)를 대상으로 교차빈도에 대한 기술통계량을 제공
# 교치빈도에 대한 통계적 유의성을 검증해 주는 분석기법

# 분산이 퍼져있는 모습을 분포로 만든 것이 카이제곱 분포
# X² = ∑(관측값 - 기대값)² / 기대값 (카이제곱 공식)
...
print(ctab)     # 관찰값(빈도)
# 기대도수 = (각행의 주변합) * (각열의 주변합) / 총합<전체 표본수>
...
# 그러므로 '벼락치기 공부하는 것과 합격여부는 관계가 없다.' 는 의견은 유지가 됩니다.

# 검정방법2 : p-value 사용
import scipy.stats as stats
chi2, p, dof, excepted = stats.chi2_contingency(ctab)
print(chi2, p)      # chi2 : 3.0     p : 0.5578254003710748
# 판정 : 유의수준 0.05 < p : 0.557825 이므로 귀무채택!
# 검정에 사용된 자료는 우연히 발생한 자료라고 할 수 있다.

# chi2 검정 정식명칭 : Perason's chi-suqare test
# 두 개의 불 연속변수(범주형) 간의 상관관계를 측정. 관찰빈도가 통계적으로 유의한지 확인