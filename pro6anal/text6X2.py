...
print(ctab)
ctab.index = ['조사방법1', '조사방법2', '조사방법3']
ctab.columns = ['매우만족', '만족', '보통', '불만족', '매우불만족']
print(ctab)

chi2, p, dof, expected = stats.chi2_contingency(ctab)
print(f"chi2:{chi2}, p:{p}, dof:{dof}")
# chi2:6.5446678, p:0.5864574, dof:8
print("expected : \n", expected)    # 예측비율

# 해석 : 유의수준 0.05 < p:0.58645743 이므로 귀무 채택. 우연히 발생한 자료라고 할 수 있다.

print("_____________________________")
# 동질성 검정 실습2) 연령대별 sns 이용률의 동질성 검정
# 20대에서 40대까지 연령대별로 서로 조금씩 그 특성이 
# 조사한 자료를 바탕으로 연령대별로 홍보전략을 세우고자 한다
# 연령대별로 이용 현황이 서로 동일한지 검정해 보도록 하자

# 귀무 : 연령대별로 sns 서비스별 이용률 현황은 동일하다.
# 대립 : 연령대별로 sns 서비스별 이용률 현황은 동일하지않다.
...
# 연령대별로 sns 서비스별 이용률 현황은 동일하지않다. 의견을 인정

print("전체 건수 : ", len(data))
# 위 자료는 샘플 자료이겠으나 모집단이라고 가정하고 샘플링 후 검정
