...
# Boosting은 약한 분류기에 대해 샘플의 일부를 보완해가며 순차적으로 학습

# brest_cancer_dataset
import numpy as np
import pandas as pd
...
from lightgbm import LGBMClassifier # xgboost 보다 성능 우수하나 자료가 적으면 과적합
...
# 레이블 분포 :  {'malignant'(악성) : 212, 'benign'(양성) : 357}
...
print(f'LGBClassifier acc : {accuracy_score(y_test, pred_lgb):.5f}')

# 피처 중요도 : gain으로 기준으로 통일
...
print((comp_df.head(10)))   # 중요 피처(변수) top10
...
xmax = float(np.ceil(top.max().max()))  # 두 모델의 최대 값
