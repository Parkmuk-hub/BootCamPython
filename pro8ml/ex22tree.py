...
import numpy as np
#  n_redundant=0 불필요한 변수 0개, random_state 난수 고정, n_informative=2 의미있는 변수 2개
# n_features=2 독립변수 2개
x, y = make_classification(n_samples=100, n_features=2, n_redundant=0,
                        n_informative=2, random_state=42)

model = DecisionTreeClassifier(criterion='gini', max_depth=3)   # max_depth=3 최대깊이 3 [자료구조 관련]
model.fit(x, y)     # 입력데이터 x, 정답데이터 y <-- supervised learning [지도 학습] 문제 + 답 제공
