#   RandomForest- 다항분류 - iris dataset
import pandas as pd
import numpy as np
...
# 이를 softmax regression 또는 multinomial logistic regression 이라고 부른다.
...
# #  Scaling (데이터 크기 표준화 - 최적화 과정에서 안정성, 수렴속도 향상)
# sc = StandardScaler()
# sc.fit(x_train)
# sc.fit(x_test)
...
# 모델에 패널티 적용 - C : L2규제. 숫자 값을 조정해가며 정확도 확인
# 최적화 알고리즘 - solver='lbfgs', softmax 지원
# multi_class='multinomial' softmax 지원함. 'auto'
print(model)
...
print('분류 정확도 확인 1')
print(f"{accuracy_score(y_test, y_pred)}")  # 0.9777777777777777 => 98% ?
