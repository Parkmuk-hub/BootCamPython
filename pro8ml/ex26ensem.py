# 앙상블 학습 : 여러 개의 분류기를 생성하고, 그 예측을 결합함으로써 보다 정확한 예측을 결과를 얻음 
# 강력한 하나의 모델보다는 약한 모델 여러 개를 조합하여 더 정확한 예측 결과를 도출.
...
print(x[:3])
print(y[:3])
print('doagnosis(y) : ', np.unique(y))  # doagnosis(y) :  [0 1]
...
# stratify : train, test 비율 유지 (불균형 데이터 모델 평가 시 왜곡 방지)
y_li = y.tolist()
