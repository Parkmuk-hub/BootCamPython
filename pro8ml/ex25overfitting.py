...
print('예측값 : ', pred2)
print('실제값 : ', y_test)
print('분류 정확도 : ', accuracy_score(y_test, pred2))  # 분류 정확도 :  0.9555555555555556
...
# KFold 학습시 전체 150행이 학습데이터(4/5, 120개), 검증데이터(1/5, 30개)로 분할되어 학습함

n_iter = 0
# KFold 객체의 split()을 호출하면 Fold 별 학습용, 검증용 테스트의 행 인덱스를 array로 변환
for train_index, test_index in kfold.split(features) :
    ...
    pred3 = dt_clf2.predict(xtest)  # test로 검증
    n_iter += 1
    # 반복할 때 마다 정확도 출력
    acc = np.round(accuracy_score(ytest, pred3), 5)
...
# 참고 : StratifiedKFoldt - 불균형한 분포도를 가진 레이블 데이터 집합을 처리하기 위한 KFold
...
grid_dtree = GridSearchCV(estimator=dt_clf2, param_grid=parameters, 
                        cv=3, refit=True)
grid_dtree.fit(x_train, y_train)    # 내부적으로 복수 개의 모형을 생성하고, 이를 실행시켜 최적의 파라미터 찾아줌
# 내부적으로 복수 개의 모형을 생성하고, 이를 실행시켜 최적의 파라미터를 찾아줌
# best_score, best_params, best_esmator, gird_score ....
...
# 과적합 방지 기타 : 불필요한 변수를 제거, 정규화(L1, L2), 데이터양 증가, 조기종료 ...
# Logit -> Decision -> Ensemble
