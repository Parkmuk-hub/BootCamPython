...
df = pd.read_csv('titanic_data.csv',header=None, skiprows=1, 
...
survival_counts = df.groupby('나이대',  observed=False)['Survived'].count()
...
# 1. 피봇 테이블 생성 (기본값은 소수 형태의 평균)
pivot_s = df.pivot_table(values='Survived', index=['Sex', '나이대'], columns='Pclass')
pisam = (pivot_s * 100).round(2)
print(pisam)
