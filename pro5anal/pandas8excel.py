...
print(root)     # <Element '{current}current' at 0x000001A37D0B3B50>

# {current} namespace 제거
for elem in root.iter() :
    if '}' in elem.tag :
        elem.tag = elem.tag.split('}', 1)[1]    # '}'를 기준으로 자르고 태그명만 남김
# {current}weather -> weather

weather = root.find('weather')
year = weather.get('year')      # 속성 값 얻기
