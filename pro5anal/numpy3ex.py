...
# 1. 정규분포 난수로 5행 4열 배열 생성
# data = np.random.randn(5, 4)

# # 2. 행 단위(axis=1) 합계와 최댓값 계산
# row_sums = data.sum(axis=1)   # 결과: [1행합, 2행합, 3행합, 4행합, 5행합]
# row_maxs = data.max(axis=1)   # 결과: [1행최대, 2행최대, 3행최대, 4행최대, 5행최대]

# # 3. 인덱스를 직접 사용하여 출력 (가장 기본적인 방식)
# # 행의 개수가 5개이므로 range(5)를 사용합니다.
# for i in range(5):  # 0 ~ 4 까지
#     # i는 0, 1, 2, 3, 4 순서로 변합니다.
#     # 출력할 때는 i + 1을 해서 1행, 2행... 순으로 보이게 합니다.
#     print(str(i + 1) + "행 합계   : " + str(row_sums[i]))
#     print(str(i + 1) + "행 최댓값 : " + str(row_maxs[i]))
...
zarr = np.zeros((6, 6)) # zarr안에 6행 6열의 zero 행렬 객체 생성
cnt = 0
for i in range(6) :
    for j in range(6) :
        cnt += 1
...
print(zarr)     # 조건1
print()
print(zarr[1, :])   # print(zarr[1]) [행, 열] 조건2
print()
print(zarr[:, 4])   # 조건3
print()
print(zarr[2:5, 2:5])   # 조건4
...
print(arr3)
print("기술 통계량 ----------")
print("평균 : ", np.mean(arr3))     # == arr3.mean()
print("합계 : ", np.sum(arr3))

print("표준 편차 : ", np.std(arr3))
print("분산 : ", np.var(arr3))

print("최대 : ", np.max(arr3))
print("최소 : ", np.min(arr3))

print("1사분위수 : ", np.percentile(arr3, 25))
print("2사분위수 : ", np.percentile(arr3, 50))
print("3사분위수 : ", np.percentile(arr3, 75))

print("누적합 : ", np.cumsum(arr3))
print()
...
result = a * b      # 알아서 브로드캐스팅함
print(result)
print(result[result >= 30])     # (result >= 30) 이러면 True/False로 나옴
print()
...
# Q5) 정규분포(평균(loc) 50, 표준편차(scale) 10)를 따르는 난수 1000개를 만들고, 상위 5% 값만 출력하세요.
