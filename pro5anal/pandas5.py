...
df = pd.read_fwf('https://raw.githubusercontent.com/pykwon/python/refs/heads/master/testdata_utf8/data_fwt.txt', header=None, widths=(10,3,5), names=('data','name','price'), encoding='utf8')
print(df)
print(df.iloc[:,0])
#print(df['date'])

print('\nchuck : 대량의 데이터를 부분씩 메모리로 읽어 처리')
# 대용량 자료 로딩 시 초과 오류 발생 방지 : 메모리를 절약
...
for bar, time_val in zip(bars, times) :
    plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
            f'{time_val:3f}초', ha='center', va='bottom', fontsize=10)
plt.ylabel('처리 시간(초)')
plt.grid(linestyle='--')
plt.tight_layout()
plt.show()