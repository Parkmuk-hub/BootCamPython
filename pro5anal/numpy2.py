...
print(np.add(x, y)) # numpy 함수(유니버셜 Function)(빠름)
...
print('\ndot는 numpy 모듈의 함수나 배열 객체의 인스턴트 메소드로 사용이 가능')
v = np.array([9, 10])
w = np.array([11, 12])
...
print('\n전치(Transpose) - 2차원 배열에 행과 열의 위치를 바꿈')
print(x)
print(x.T)
print(x.transpose())
...
np.savetxt("my.txt", x)     # 배열 file i/o 불러오기 => loadtxt()
