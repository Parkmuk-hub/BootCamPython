...
# 자바와 달리 접근 지정자가 없다. 메소드 오버로딩이 없다.
# 모듈의 멤버 : 변수, 명령문, 함수, 모듈, 클래스
print('뭔가를 하다 객체 만들기')

class TestClass :
    aa = 1 # 멤버필드(변수) : 현재 클래스 내에서 전역이다.

    def __init__(self) :
        print('생성자 : 객체 생성시 가장 먼저 1회만 호출 - 초기화 담당') # 특별한 메소드

    def __del__(self) :
        print("소멸자 : 프로그램 종료시 자동실행 - 마무리 작업")
    
    def printMsg(self) : # 일반 메소드
        name = '한국인' # 지역 변수 : printMsg 내부에서만 작동
        print(name)
...
TestClass.printMsg(test) # 2. UnBound 
