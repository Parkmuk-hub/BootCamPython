...
with open('mydb.dat', mode = 'rb') as obj:
    config = pickle.load(obj)
open('mydb.dat', 'rb'): mydb.dat 파일을 바이너리 읽기(rb) 모드로 엽니다.

as obj: 열린 파일 객체를 이제부터 obj라는 이름으로 부르겠다는 뜻입니다.

pickle.load(obj): obj 파일에서 피클 데이터를 읽어옵니다.

블록이 끝나는 순간: (코드 줄이 왼쪽으로 다시 돌아가는 순간) obj.close()가 자동으로 호출됩니다.
...
def chulbal() :
    try :
        ...
        where busernum = {0} 
        """.format(bu_no)   # 0번째로 들어감 {0,1} 이면 (bu_no, 변수)
        # print(sql)

        cursor.execute(sql)     # db서버로 가서 내용을 가지고 RAM [주기억장치]으로 감
