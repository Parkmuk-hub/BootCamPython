# 개인용 Database : sqlite3 - Python의 기본 모듈
...
conn = sqlite3.connect(':memory:')          # RAM에만 db 저장. 휘발성 = 일회용
...
try : # db는 이것이 무조건 필요
    ...
    print(cur.fetchall())
    print()
    cur.execute("select name, addr, phone from friends")    # <== 원본 상관없이 내맘대로 읽어오게 가능
    for r in cur :
        #print(r)
        print(r[0] + ' ' + r[1] + ' ' + r[2])
