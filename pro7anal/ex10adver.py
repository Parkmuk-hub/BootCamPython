...
print(0.782224 ** 2)    # r-squared 값 나옴
print(lm.summary())
print(lm.summary().tables[1])   # 끊어서 볼 수 있다.
print()
...
print('예측값 : ', lm.predict(x_new).values)    # 17.97077451
print('직접 계산 : ', lm.params.tv * 230.1 + lm.params.Intercept)   # 17.970774512765537

# 경험하지 않은 tv 광고비에 따른 상품 판매량
my_new = pd.DataFrame({'tv':[100, 200, 300]})
...
# 잔차(Residual) : 실제 관측값과 모델이 예측한 값의 차이를 의미한다.
# 모델이 데이터를 얼마나 잘 설명하는지 보여주는 척도
fitted = lm.predict(advdf) # <= 매우 추천      # lm.predict(advdf.tv)
residual = advdf['sales'] - fitted
print('실제값 : ', advdf['sales'].values)
...
print(f"통계량:{stats}, p-value:{p}")   
# 통계량:0.9905306561484953, p-value : 0.21332551 > 0.05 이므로 만족
print('정규성 만족' if p > 0.05 else "정규성 위배")
# Q-Q plot 으로 시각화
sm.qqplot(residual, line='s')
plt.title("Q-Q plot으로 정규성 만족")
plt.show()
...
bp_test = het_breuschpagan(residual, lm.model.exog) # or sm.add_constant(advdf['tv])
...
# 데이터가 적을 때, 이상치가 의심스어울 때, 모델 결과가 이상하게 나올 때...
from statsmodels.stats.outliers_influence import OLSInfluence
cd, _ = OLSInfluence(lm).cooks_distance     # Cook 거리, 인덱스 반환
...
# Cook 거리가 가장 큰(영향력이 큰) 관측치 원본 확인
