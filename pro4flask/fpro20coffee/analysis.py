...
def analysiy_func(rdata:list[dict]) :
    ...
    if min_expected < 5:
        note = f"<br><small>* 주의 : 기대빈도 최소 값 {min_expected:.2f} (5 미만)</small>"

    if p >= alpha:
        results = f"p값 {p:.5f} >= {alpha} : 성별에 따라 커피 선호 브랜드는 <b>차이가 없다.</b>"

    else :
        results = f"p값 {p:.5f} < {alpha} : 성별에 따라 커피 선호 브랜드는 <b>차이가 있다.</b>"
    
    return crossTab, results, df

def save_bar_chart_func(df:pd.DataFrame, out_path:Path) -> bool :
