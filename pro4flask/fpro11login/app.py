...
def gogek_list(jikwonno:int):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # [수정] return이 with 블록 안에 있어야 안전합니다.
            return render_template("gogek_list.html", customers=customers, 
                                empno=jikwonno, empname=(emp["jikwonname"] if emp else ""))
    finally:
        conn.close()
