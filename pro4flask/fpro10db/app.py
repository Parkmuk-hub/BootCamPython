...
# flash : 임시 메세지 출력용 (내부적으로 session에 저장해둠 - secret_key 필요)
# pip install pymysql
...
def show_list() :
    ...
    # except pymysql.err.IntegrityError as e : 도 가능함
        # ...
...
def add_form():
    messages = list(get_flashed_messages())
    return render_template("form_add.html", message=messages)  # 추가 폼 처리
...
def edit_form(code:int) :   # 수정 폼 호출
    ...
    try :
        with conn.cursor() as cur :
            cur.execute("select * from sangdata where code=%s", (code,))    # 튜플 타입
            row = cur.fetchone()
        if not row:
            flash("해당 자료가 없어요")
            return redirect(url_for("show_list"))
...
def delete_row(code:int) :
    ...
    try :
        ...
        conn.rollback()
        flash(f"삭제 실패 : {e}")
        return redirect(url_for("show_list"), code=code)
