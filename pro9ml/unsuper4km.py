...
# k-평균 군집화(K-means Clustering) 알고리즘은 군집의 중심이 되는 k개의 seed(씨드) 점들을 선택하여 
# 그 seed점(중심점)과 거리가 가까운 개체들을 그룹화하는 방법이다. 알고리즘은 다음과 같다.
# 1) K개의 중심점을 임의로 배치한다.
...
init_centroid = 'k-means++' # cluster의 중심을 k-means++로 선택, 중심을 최대한 멀리
...
pred = kmodel.fit_predict(x)    # 클러스터링으로 구분한 결과
print('pred : ', pred)
...
# 시각화
plt.scatter(x[pred == 0, 0], x[pred == 0, 1], c='red', marker='o', s=50, label='clusters1')
plt.scatter(x[pred == 1, 0], x[pred == 1, 1], c='green', marker='s', s=50, label='clusters2')
plt.scatter(x[pred == 2, 0], x[pred == 2, 1], c='blue', marker='v', s=50, label='clusters3')
plt.scatter(kmodel.cluster_centers_[:, 0], kmodel.cluster_centers_[:, 1], c='black', marker='+', 
            s=60, label='center')   # 각 군집 중심점 표시
plt.legend()
plt.grid(True)
plt.show()
