...
# k=3이므로 71, 82, 31 번 자료가 분류에 참여함.
# 새로운 자료와 거리는 0.223, 0.3, 0.435

print('\n\n비지도 학습 - K평균 비계층 군집 알고리즘')
from sklearn.cluster import KMeans
kmeansModel = KMeans(n_clusters=3, init='k-means++', random_state=0)
kmeansModel.fit_predict(train_x)   # label이 주어지지 않는다.
print(kmeansModel.labels_)      # 군집 라벨 출력
# print('예측군집값 : ', kmeansModel.labels_[:10]) 
# print('실제값 : ', train_y[:10])
print()
#군집별 자료보기
print('0 cluster : ', train_y[kmeansModel.labels_ == 0])    # 0번째 군집은 라벨 2
print('1 cluster : ', train_y[kmeansModel.labels_ == 1])    # 1번째 군집은 라벨 0
print('2 cluster : ', train_y[kmeansModel.labels_ == 2])    # 2번째 군집은 라벨 1
# 새로운 값 군집 분류
import numpy as np
new_input = np.array([[6.1, 2.8, 4.7, 1.2]])
