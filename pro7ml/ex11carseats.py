...
print(lm.summary())     # Adj. R-squared: 0.364 == 36.4% 유의함
...
# 계량:0.9949221268962882, p-value:0.21270047355494398
...
print("\n등분산성 검정 : 모든 x값에서 오차의 퍼짐이 유사해야 한다.")
from statsmodels.stats.diagnostic import het_breuschpagan
bp_test = het_breuschpagan(residual, sm.add_constant(df['Sales'])) # or sm.add_constant(advdf['tv])
...
# 값의 범위는 0 ~ 4이고 2이면 정상(자기 상관 없음). < 2 이면 양의 자기상관, > 2 이면 음의 자기상관
# model.summary()로 확인가능   
import statsmodels.api as sm
print("Durbin-Watson : ", sm.stats.stattools.durbin_watson(residual))       
# Durbin-Watson :  1.9314981270829592 이므로 잔차의 자기상관은 없다.
