...
y_lin_fit = model.predict(x_fit)    # 그래프에 사용
...
cu_r2 = r2_score(y, model.predict(x_cubic))
print('cu_r2 : ', cu_r2)    # 0.6578476405895719

plt.scatter(x, y, label='초기 데이터')
plt.plot(x_fit, y_lin_fit, linestyle=":", label='liner fit(d=1), $R^2=%.2f$'%model_r2, c="y", lw=3)
plt.plot(x_fit, y_quad_fit, linestyle=":", label='quad fit(d=1), $R^2=%.2f$'%q_r2, c="r", lw=3)
plt.plot(x_fit, y_cubic_fit, linestyle=":", label='cubic fit(d=1), $R^2=%.2f$'%cu_r2, c="k", lw=3)
plt.xlabel("하위계층 비율")
plt.ylabel("주택가격")
plt.legend()
plt.show()  # 음의 상관관계